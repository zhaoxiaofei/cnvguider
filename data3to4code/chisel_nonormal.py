...
def sequencing_paired(args, read, fragment, sdfragment):
    ...
    # -ss can be in conflict with -l
    cmd = '{}        -na -i {} -p -l {} -f {} -m {} -s {} -o {}'.format(args['art'],          args['reference'], read, args['simcov'], fragment, sdfragment, pre_fastq)
    ...
    if proc.returncode != 0 or not (os.path.exists(fastqs[0]) and os.path.exists(fastqs[1])):
        ...
        raise ValueError(error('ART Illumina simuation of sequencing reads failed:\n{}\n{}\n{}\n{}\n'.format(stdout, stderr, cmd, shlex.split(cmd))))
    ...
    returncode = os.system('( {} | {} ) 1> {} 2> {} '.format(cmd_bwa, cmd_sor, stdout, stderr))
    if returncode != 0 or not os.path.exists(args['normal']):
    ...
    fastqs = ('{}1.fq'.format(pre_fastq), '{}2.fq'.format(pre_fastq))
    #if sor.returncode != 0: # this is a bug
    if sam.returncode != 0: # fixed the bug
        #raise ValueError(error('ART Illumina simuation of sequencing reads failed:\n{}\n{}\n'.format(stdout, stderr)))
        raise ValueError(error('ART Illumina simuation of sequencing reads failed:\n{}\n{}\n{}\n{}\n'.format(stdout, stderr, cmd_sam, shlex.split(cmd_sam))))
...
def sequencing_single(args, read):
    ...
    # -ss can be in conflict with -l
    cmd = '{}        -na -i {} -l {} -f {} -o {}'.format(args['art'],          args['reference'], read, args['simcov'], pre_fastq)
    with open(os.path.join(tmpdir, 'art_illumina.log'), 'w') as simlog:
        proc = sp.Popen(shlex.split(cmd), stdout=sp.PIPE, stderr=simlog)
        stdout, stderr = proc.communicate()
